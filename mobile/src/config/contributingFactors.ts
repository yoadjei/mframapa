 ...
 * own 4-tier scale instead (see factorFillColor/factorStatusKey in
 * theme/colors.ts), by design. Mirrors frontend-pwa's
 * contributingFactors.config.js exactly.
 */

// Priority order: drop from the end when a factor's data is unavailable,
// never reorder the remaining ones.
export const FACTOR_ORDER = ['no2', 'o3', 'so2', 'co'] as const;
export const MAX_FACTOR_CARDS = 4;

export const FACTOR_TINY_LABEL_KEY: Record<string, string> = {
  no2: 'factor.tiny.no2',
  o3: 'factor.tiny.o3',
  so2: 'factor.tiny.so2',
  co: 'factor.tiny.co',
